...
)
{
  ...
  mLeastSquaresFunction = 
    std::make_shared<Plato::Elliptic::CriterionEvaluatorLeastSquares<PhysicsType>>(aSpatialModel, mDataMap);
...
)
{
  ...
  //
  Plato::Matrix<3,3> tGoldInertiaTensorAboutCG = 
    Plato::plus(tGoldInertiaTensor, tParallelAxisTheoremMatrix, -Mass);
  //
  ...
  mMinusRotatedParallelAxisTheoremMatrix = 
    Plato::times(-1.0,Plato::times(Plato::transpose(mInertiaRotationMatrix), Plato::times(tParallelAxisTheoremMatrix, mInertiaRotationMatrix)));
}
...
{
  std::shared_ptr<Plato::Elliptic::CriterionEvaluatorScalarFunction<PhysicsType>> tMassFunction =
  ...
  for(const auto& tDomain : mSpatialModel.Domains)
  {
    auto tName = tDomain.getDomainName();
    // criterion value
    std::shared_ptr<Plato::Elliptic::CriterionMassMoment<Residual>> tValue = 
    ...
    // criterion gradient wrt control
    ...
    // criterion gradient wrt configuration
    ...
    // criterion gradient wrt node states
    std::shared_ptr<Plato::Elliptic::CriterionMassMoment<GradientN>> tGradientN = 
      std::make_shared<Plato::Elliptic::CriterionMassMoment<GradientN>>(tDomain, mDataMap);
    tGradientN->setMaterialDensity(mMaterialDensities[tName]);
    tGradientN->setCalculationType(tCalculationType);
    tMassFunction->setEvaluator(Plato::evaluation_t::GRAD_N, tGradientN, tName);
  }
...
)
{
  ...
  for(const auto& tDomain : mSpatialModel.Domains)
  {
    auto tName = tDomain.getDomainName();
    //
    std::shared_ptr<Plato::Elliptic::CriterionMassMoment<Residual>> tNumeratorValue = 
      std::make_shared<Plato::Elliptic::CriterionMassMoment<Residual>>(tDomain, mDataMap);
    ...
    //
    std::shared_ptr<Plato::Elliptic::CriterionMassMoment<GradientN>> tNumeratorGradientN = 
      std::make_shared<Plato::Elliptic::CriterionMassMoment<GradientN>>(tDomain, mDataMap);
    tNumeratorGradientN->setMaterialDensity(mMaterialDensities[tName]);
    tNumeratorGradientN->setCalculationType(aMomentType);
    tNumerator->setEvaluator(Plato::evaluation_t::GRAD_N, tNumeratorGradientN, tName);
  }
  //
  ...
  //
  std::shared_ptr<Plato::Elliptic::CriterionEvaluatorDivision<PhysicsType>> tMomentOverMassRatioFunction =
...
)
{
  const std::string tInertiaName = std::string("Second Mass Moment (Moment type = ")
                                   + aMomentType + ")";
  std::shared_ptr<Plato::Elliptic::CriterionEvaluatorScalarFunction<PhysicsType>> tSecondMomentFunction =
       std::make_shared<Plato::Elliptic::CriterionEvaluatorScalarFunction<PhysicsType>>(aSpatialModel, mDataMap);
  ...
  for(const auto& tDomain : mSpatialModel.Domains)
  {
    ...
    std::shared_ptr<Plato::Elliptic::CriterionMassMoment<GradientN>> tGradientN = 
      std::make_shared<Plato::Elliptic::CriterionMassMoment<GradientN>>(tDomain, mDataMap);
    tGradientN->setMaterialDensity(mMaterialDensities[tName]);
    tGradientN->setCalculationType(aMomentType);
    tSecondMomentFunction->setEvaluator(Plato::evaluation_t::GRAD_N, tGradientN, tName);
  }
...
{
  std::shared_ptr<Plato::Elliptic::CriterionEvaluatorWeightedSum<PhysicsType>> tMomentOfInertiaFunction = 
    std::make_shared<Plato::Elliptic::CriterionEvaluatorWeightedSum<PhysicsType>>(aSpatialModel, mDataMap);
...
{
  Plato::ScalarVector tGradientN = mLeastSquaresFunction->gradientNodeState(aDatabase,aCycle);
  return tGradientN;
}
