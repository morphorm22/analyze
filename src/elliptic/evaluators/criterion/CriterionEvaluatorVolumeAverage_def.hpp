...
)
{
  ...
  for(const auto& tDomain : mSpatialModel.Domains)
  {
    ...
    std::shared_ptr<Plato::CriterionBase> tGradientN = 
      tFactory.template createScalarFunction<GradientN>(tDomain,mDataMap,aInputParams,tFunctionType,mFunctionName);
    tGradientN->setSpatialWeightFunction(mSpatialWeightingFunctionString);
    tVolumeFunction->setEvaluator(Plato::evaluation_t::GRAD_N, tGradientN, tName);
  }
...
)
{
  ...
  for(const auto& tDomain : mSpatialModel.Domains)
  {
    ...
    std::shared_ptr<Plato::CriterionBase> tNumeratorGradientN = 
      tFactory.template createScalarFunction<GradientN>(tDomain,mDataMap,aInputParams,tFunctionType,mFunctionName);
    tNumeratorGradientN->setSpatialWeightFunction(mSpatialWeightingFunctionString);
...
{
  Plato::ScalarVector tGradientN = mDivisionFunction->gradientNodeState(aDatabase,aCycle);
  return tGradientN;
}
