...
namespace Elliptic
{
    ...
    ) 
    {
        ...
        {
            return std::make_shared<CriterionEvaluatorMassProperties<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        if(tFunctionType == "Least Squares")
        {
            return std::make_shared<CriterionEvaluatorLeastSquares<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        if(tFunctionType == "Weighted Sum")
        {
            return std::make_shared<CriterionEvaluatorWeightedSum<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        if(tFunctionType == "Solution")
        {
            return std::make_shared<CriterionEvaluatorSolutionFunction<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        if(tFunctionType == "Division")
        {
            return std::make_shared<CriterionEvaluatorDivision<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        if(tFunctionType == "Volume Average Criterion")
        {
            return std::make_shared<CriterionEvaluatorVolumeAverage<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        if(tFunctionType == "Scalar Function")
        {
            return std::make_shared<CriterionEvaluatorScalarFunction<PhysicsType>>(aSpatialModel, aDataMap, aProblemParams, aFunctionName);
        }
        else
        {
            return nullptr;
